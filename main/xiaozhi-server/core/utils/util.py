...
def check_ffmpeg_installed() -> bool:
    ...
    Returns:
        bool: nếu ffmpeg 正常可用，quay lại True；否则抛出 ValueError Ngoại lệ。
...
def extract_json_from_string(input_string):
    ...
    if match:
        return match.group(1)  # 返回提取的 JSON Chuỗi
    return None
...
def audio_to_data_stream(
    ...
    # 读取音频文件，-nostdin Tham số：不要从标准输入读取数据，否则FFmpeg会阻塞
