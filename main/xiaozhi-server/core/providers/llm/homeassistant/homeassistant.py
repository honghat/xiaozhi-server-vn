...
class LLMProvider(LLMProviderBase):
    ...
    def response(self, session_id, dialogue, **kwargs):
        try:
            ...
            # 提取最后一个 role để 'user' 的 content
            input_text = None
            if isinstance(dialogue, list):  # 确保 dialogue 是一个列表
                # 逆序遍历，找到最后一个 role để 'user' 的消息
                for message in reversed(dialogue):
                    if message.get("role") == "user":  # 找到 role để 'user' 的消息
            ...
            payload = {
                ...
                "conversation_id": session_id,  # sử dụng session_id 作为 conversation_id
            }
            # 设置请求头
            headers = {
